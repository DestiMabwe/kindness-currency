...
A single shared component, `TabPills` (`src/components/shared/TabPills.tsx`), is the only tab pattern in the app — reach for it, never a second bespoke implementation. Two pill buttons per instance, same shape/color language as the filter pills used elsewhere (inactive: `#F0ECE4` fill, Deep Ink text; active: Kindness Red fill, white text), with proper `tablist`/`tab`/`tabpanel` ARIA roles since each use genuinely switches between distinct content, not filters one list. Two uses today: Profile's Sent/Received view, and AuthGate's Sign Up/Log In mode switch. Still deliberate, not a general-purpose pattern for arbitrary in-page switching — a new use should genuinely be "two distinct panels," not a filter dressed up as a tab.
